...
export const ChatWidget: React.FC<Props> = ({ username }) => {
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-[2rem] h-full flex flex-col shadow-2xl overflow-hidden">
      <div className="p-5 border-b border-slate-800 bg-slate-800/50 flex items-center justify-between">
         <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest leading-none mb-1">Subject AI Assistant</p>
              <h4 className="text-xs font-black text-white uppercase italic">@ {username}</h4>
            </div>
         </div>
          ...
          <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[85%] px-4 py-3 rounded-2xl text-xs leading-relaxed ${
              m.role === 'user' 
                ...
                : 'bg-slate-800 text-slate-300 font-medium rounded-tl-none border border-slate-700'
            }`}>
              {m.text}
            </div>
          </div>
        ...
        {loading && (
          <div className="flex justify-start">
            <div className="bg-slate-800 px-4 py-3 rounded-2xl flex items-center gap-2">
              ...
              <span className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Analyzing...</span>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="p-4 border-t border-slate-800 bg-slate-900">
        <div className="relative">
          <input 
            ...
            className="w-full bg-slate-800 border border-slate-700 rounded-xl py-3 pl-4 pr-12 text-xs text-white focus:outline-none focus:ring-1 focus:ring-blue-500"
          />
          <button 
            onClick={handleSend}
