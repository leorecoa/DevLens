...
export const AnalysisDashboard: React.FC<Props> = ({ 
  ...
  const getSeniorityColor = (seniority: string) => {
    switch (seniority) {
      case 'Senior': return 'text-emerald-400 border-emerald-500/30 bg-emerald-500/5';
      case 'Architect': return 'text-purple-400 border-purple-500/30 bg-purple-500/5';
      case 'Lead': return 'text-blue-400 border-blue-500/30 bg-blue-500/5';
      default: return 'text-yellow-400 border-yellow-500/30 bg-yellow-500/5';
    }
  };
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-[2rem] backdrop-blur-md">
        ...
        <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-[2rem] backdrop-blur-md flex flex-col">
          <h3 className="text-xs font-black text-slate-500 uppercase tracking-widest mb-4 flex items-center gap-2">
            <ShieldCheck size={14} className="text-emerald-500" /> Executive Intelligence
          </h3>
          <p className="text-slate-300 italic leading-relaxed text-sm mb-6 flex-1">
          ...
          <div className="space-y-3 no-print">
             <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="relative">
                  ...
                  {showPipelineMenu && (
                    <div className="absolute bottom-full mb-2 w-full bg-slate-800 border border-slate-700 rounded-xl overflow-hidden shadow-2xl z-50 animate-in slide-in-from-bottom-2 duration-200">
                      <div className="p-2 border-b border-slate-700">
                        <p className="text-[10px] font-black text-slate-500 uppercase px-2 py-1">Select Pipeline Folder</p>
                      </div>
                      <div className="max-h-48 overflow-y-auto custom-scrollbar">
                        {folders.length === 0 ? (
                          <div className="p-4 text-center">
                            <p className="text-xs text-slate-400 italic mb-2">No folders found</p>
                            <button 
                          ...
                          folders.map(f => (
                            <button 
                              ...
                              className="w-full text-left px-4 py-3 text-xs text-slate-200 hover:bg-slate-700 flex items-center gap-2"
                            >
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div className="relative">
                    <button 
                      ...
                      className="w-full bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 font-black py-3 rounded-xl flex items-center justify-center gap-2 text-[10px] uppercase tracking-widest transition-all"
                    >
                      <Share2 size={14} />
                      Share
                    </button>
                    {showShareMenu && (
                      <div className="absolute bottom-full right-0 mb-2 w-48 bg-slate-800 border border-slate-700 rounded-xl overflow-hidden shadow-2xl z-50 animate-in slide-in-from-bottom-2 duration-200">
                        <div className="p-2 border-b border-slate-700">
             ...
             <button 
               onClick={() => handleSocialShare('linkedin')}
               className="w-full mt-2 bg-blue-700/20 hover:bg-blue-700/40 text-blue-400 border border-blue-600/30 font-black py-2.5 rounded-xl flex items-center justify-center gap-2 text-[10px] uppercase tracking-widest transition-all no-print"
             >
               <Linkedin size={14} />
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-slate-900/40 border border-emerald-500/20 p-6 rounded-3xl">
      ...
      <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-[2rem] backdrop-blur-md">
        ...
        <div className="flex flex-wrap gap-2">
          {analysis.techStack.map((tech, i) => (
            <span key={i} className="px-3 py-1.5 bg-slate-800 border border-slate-700 rounded-lg text-xs text-slate-300 font-bold hover:border-blue-500/50 hover:text-white transition-colors cursor-default">
              {tech}
            </span>
          ))}
        </div>
      ...
      <div className="bg-slate-900/60 border border-slate-800 p-6 rounded-[2rem] backdrop-blur-md">
        <div className="flex items-center justify-between mb-6">
          ...
          <a href={`https://github.com/${username}?tab=repositories`} target="_blank" rel="noreferrer" className="text-[10px] font-black text-blue-400 uppercase hover:underline flex items-center gap-1 no-print">
            Explore All <ChevronRight size={12} />
          </a>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {repositories.slice(0, 6).map((repo, i) => (
            <div key={i} className="p-4 bg-slate-800/50 border border-slate-700/50 rounded-2xl hover:border-slate-600 transition-all group">
              <div className="flex items-center justify-between mb-2">
                <h4 className="text-sm font-black text-slate-200 group-hover:text-blue-400 truncate max-w-[150px]">{repo.name}</h4>
                <div className="flex items-center gap-3">
              ...
              <p className="text-[11px] text-slate-400 line-clamp-2 mb-3 h-8">{repo.description || "No descriptive intelligence provided."}</p>
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-black uppercase text-slate-500">{repo.language || 'Hybrid'}</span>
                <span className="text-[9px] font-bold text-slate-600">Updated {new Date(repo.updated_at).toLocaleDateString()}</span>
              </div>
            </div>
          ))}
        </div>
